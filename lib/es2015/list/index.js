...
const _empty = state_2.emptyState(false);
function emptyList() {
    return _empty;
}
...
function set(index, value, list) {
    return _exec(list, state => values_1.setValueAtOrdinal(state, index, value));
}
exports.set = set;
...
function appendArray(values, list) {
    return values.length === 0 ? list
        : _exec(list, state => values_1.appendValues(state, values));
}
...
function prependArray(values, list) {
    return values.length === 0 ? list
        : _exec(list, state => values_1.prependValues(state, values));
}
exports.prependArray = prependArray;
function insert(index, value, list) {
    return _exec(list, state => values_1.insertValues(state, index, [value]));
}
exports.insert = insert;
function insertArray(index, values, list) {
    if (values.length === 0)
        return list;
    return _exec(list, state => values_1.insertValues(state, index, values));
}
exports.insertArray = insertArray;
function remove(index, list) {
    return list.size === 0 ? list
        : _exec(list, state => values_1.deleteValues(state, index, index + 1));
}
exports.remove = remove;
function removeRange(start, end, list) {
    return list.size === 0 ? list
        : _exec(list, state => values_1.deleteValues(state, start, end));
}
exports.removeRange = removeRange;
function pop(list) {
    return list.size === 0 ? list
        : _exec(list, state => slice_1.sliceList(state, 0, -1));
}
exports.pop = pop;
function popFront(list) {
    return list.size === 0 ? list
        : _exec(list, state => slice_1.sliceList(state, 1, state.size));
}
exports.popFront = popFront;
exports.shift = popFront;
function skip(count, list) {
    return list.size === 0 || count === 0 ? list
        : _exec(list, state => slice_1.sliceList(state, count, state.size));
}
exports.skip = skip;
function take(count, list) {
    return list.size === 0 || count >= list.size ? list
        : _exec(list, state => slice_1.sliceList(state, 0, count));
}
exports.take = take;
function slice(start, end, list) {
    ...
    return list.size === 0 ? list
        : _exec(list, state => slice_1.sliceList(state, start, end));
}
exports.slice = slice;
function concat(left, right) {
    return _exec(left, state => concat_1.concatLists(state, state_2.cloneState(right, ownership_1.nextId(), true)));
}
...
function concatMany(lists) {
    ...
    switch (lists.length) {
        case 1:
            other = lists[1];
            _exec(list, state => concat_1.concatLists(state, state_2.cloneState(other, ownership_1.nextId(), true)));
        default:
