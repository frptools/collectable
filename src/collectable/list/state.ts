...
export function cloneState<T>(state: PListState<T>, group: number, mutable: boolean): PListState<T> {
  return new PListState<T>(group, mutable ? batch.owner || -1 : 0, state.size, state.lastWrite, state.left, state.right);
}

export function ensureMutable<T>(state: PListState<T>, ): PListState<T> {
  return isMutable(state.owner) ? state : cloneState(state, nextId(), true);
}

export function ensureImmutable<T>(state: PListState<T>, done: boolean): PListState<T> {
...
export function getView<T>(state: PListState<T>, anchor: OFFSET_ANCHOR, asWriteTarget: boolean, preferredOrdinal: number = -1): View<T> {
...
export function getOtherView<T>(state: PListState<T>, anchor: OFFSET_ANCHOR): View<T> {
  return anchor === OFFSET_ANCHOR.LEFT ? state.right : state.left;
}

export function setView<T>(state: PListState<T>, view: View<T>): void {
...
export function emptyState<T>(mutable: boolean): PListState<T> {
  return mutable
    ? new PListState<T>(nextId(), batch.owner || -1, 0, OFFSET_ANCHOR.RIGHT, View.empty<T>(OFFSET_ANCHOR.LEFT), View.empty<T>(OFFSET_ANCHOR.RIGHT))
    : _defaultEmpty;
}

var _defaultEmpty = new PListState<any>(0, 0, 0, OFFSET_ANCHOR.RIGHT, View.empty<any>(OFFSET_ANCHOR.LEFT), View.empty<any>(OFFSET_ANCHOR.RIGHT));