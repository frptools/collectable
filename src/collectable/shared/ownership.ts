...
function start(): void {
  if(_depth === 0) {
    _owner = nextId();
    log(`initialize batch ${_owner}`);
  }
  else {
    log(`enter batch ${_owner}, depth: ${_depth + 1}`);
  }
  _depth++;
}
...
function end(): boolean {
  if(_depth > 0) {
    if(--_depth > 0) {
      log(`exit batch ${_owner}, depth: ${_depth}`);
      return false;
    }
    log(`conclude batch ${_owner}`);
    _owner = 0;
  }
  return true;
}
...
export function isMutable(owner: number): boolean {
  log(`[isMutable] owner: ${owner}, _owner: ${_owner}`);
