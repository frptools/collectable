...
import {emptyState, getOtherView} from '../../src/list/state';
import {Slot} from '../../src/list/slot';
import {sliceList} from '../../src/list/slice';
import {createArray} from '../../src/list/values';
...
suite('[List: slicing and splicing]', () => {
  suite('slice()', () => {
    ...
    test('slicing a superset of a non-empty list is a noop', () => {
      ...
      var list = PersistentList.fromArray(values)._state;
...

    {
      ...
      const list = PersistentList.fromArray(values)._state;
