...
import {CONST, COMMIT_MODE} from '../../src/list/common';
import {PersistentList} from '../../src/list';
import {ListState} from '../../src/list/state';
import {Slot} from '../../src/list/slot';
import {View} from '../../src/list/view';
import {TreeWorker} from '../../src/list/traversal';

export const BRANCH_FACTOR = CONST.BRANCH_FACTOR;
export const BRANCH_INDEX_BITCOUNT = CONST.BRANCH_INDEX_BITCOUNT;

export type ListType<T> = PersistentList<T>|ListState<T>;
...
function getState<T>(arg: ListType<T>): ListState<T> {
  return arg instanceof PersistentList ? arg._state : arg;
}

export function listOf(size: number, offset = 0): PersistentList<string> {
  return PersistentList.fromArray<string>(makeValues(size, offset));
}
