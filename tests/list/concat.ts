...
import {appendValues, createArray} from '../../src/list/values';
import {PersistentList} from '../../src/list';
import {emptyState} from '../../src/list/state';
import {Slot} from '../../src/list/slot';
import {concatLists, join} from '../../src/list/concat';
import {compact} from '../../src/list/compact';
...
suite('[List: concatenation]', () => {
  ...
  suite('concat()', () => {
    ...
    test('joins lists when both lists each have pre-existing reserved head and tail views', () => {
      ...
      var list1 = PersistentList.fromArray<any>(leftValues).prepend('X');
      var list2 = PersistentList.fromArray<any>(rightValues).prepend('Y');
