...
import {PersistentList} from '../../src/list';
...
suite('[List: public]', () => {
  suite('.empty()', () => {
    test('should have size 0', () => {
      const list = PersistentList.empty<string>();
  ...
  suite('#append()', () => {
    test('should not mutate the original List', () => {
      const empty = PersistentList.empty<string>();
    ...
    test('should have size:1 after adding the first element', () => {
      const list = PersistentList.empty<string>().append('foo');
      assert.strictEqual(list.size, 1);
      assert.deepEqual(gatherLeafValues(list), ['foo']);
    });

    test('should have size:2 after adding the second element', () => {
      const list = PersistentList.empty<string>().append('foo').append('bar');
    ...
    test('should push each additional argument as an independent value', () => {
      ...
      const list = PersistentList.empty<string>().append(...values);
  ...
  suite('#prepend()', () => {
    ...
    test('should have size:1 after adding the first element', () => {
      const list = PersistentList.empty<string>().prepend('foo');
      assert.strictEqual(list.size, 1);
      assert.deepEqual(gatherLeafValues(list), ['foo']);
    });

    test('should have size:2 after adding the second element', () => {
      const list = PersistentList.empty<string>().prepend('foo').prepend('bar');
      assert.strictEqual(list.size, 2);
    ...
    test('should push multiple arguments so that they appear in the order they were specified', () => {
      const list = PersistentList.fromArray<string>(['test']).prepend('foo', 'bar', 'baz');
  ...
  suite('#pop()', () => {
    ...
    test('should return an empty list if the input list contains only one element', () => {
      var list = PersistentList.fromArray<any>(['X']).pop();
      assert.strictEqual(list.size, 0);
    });
  });
  ...
  suite('#popFront()', () => {
    ...
    test('should return a list that excludes the last element of the input list', () => {
      var list = PersistentList.fromArray<any>(['X', 'Y', 'Z']).popFront();
      assert.strictEqual(list.size, 2);
    ...
    test('should return an empty list if the input list contains only one element', () => {
      var list = PersistentList.fromArray<any>(['X']).popFront();
      assert.strictEqual(list.size, 0);
    });
  });
  ...
  suite('#skip()', () => {
    ...
    test('should return a list that excludes the specified number of elements from the left of the input list', () => {
      ...
      var list = PersistentList.fromArray<any>(values).skip(2);
      assert.strictEqual(list.size, 4);
    ...
    test('should return an empty list if the input argument >= list.size', () => {
      var values = ['A', 'B', 'C', 'X', 'Y', 'Z'];
      var list = PersistentList.fromArray<any>(values).skip(6);
      assert.strictEqual(list.size, 0);
    });

    test('should return the same list if the input argument === 0', () => {
      var values = ['A', 'B', 'C', 'X', 'Y', 'Z'];
      var list = PersistentList.fromArray<any>(values);
  ...
  suite('#take()', () => {
    ...
    test('should return a list that excludes the specified number of elements from the left of the input list', () => {
      var values = ['A', 'B', 'C', 'X', 'Y', 'Z'];
      var list = PersistentList.fromArray<any>(values).take(4);
      assert.strictEqual(list.size, 4);
    ...
    test('should return an empty list if the input argument === 0', () => {
      var values = ['A', 'B', 'C', 'X', 'Y', 'Z'];
      var list = PersistentList.fromArray<any>(values).take(0);
      assert.strictEqual(list.size, 0);
    });
  ...
  suite('#get()', () => {
    test('should return undefined if the index is out of range', () => {
      var list = PersistentList.empty<string>();
      var listC = listOf(33);
  ...
  suite('#set()', () => {
    test('throws an error if the index is out of range', () => {
      assert.throws(() => PersistentList.empty<any>().set(0, 'X'));
      assert.throws(() => PersistentList.fromArray(['X', 'Y']).set(2, 'Z'));
      assert.throws(() => PersistentList.fromArray(['X', 'Y']).set(-3, 'Z'));
    });

    test('updates the value at the specified index', () => {
      var values = ['A', 'B', 'C', 'X', 'Y', 'Z'];
      var list1 = PersistentList.fromArray<any>(values);
      ...
      list1 = PersistentList.fromArray<any>(values);
      list2 = list1.set(0, 'J');
    ...
    test('updates the value at a location relative to the end of the list if the specified index is negative', () => {
      var list1 = PersistentList.fromArray<any>(['A', 'B', 'C', 'X', 'Y', 'Z']);
  ...
  suite('#delete()', () => {
    test('returns an identical list if the index is out of bounds', () => {
      ...
      var list0 = PersistentList.fromArray(values);
      var list1 = list0.delete(values.length);
      var list2 = PersistentList.empty();
    ...
    test('returns an empty list if the index points at the first element of a single-element list', () => {
      var list = PersistentList.fromArray(['X']).delete(0);
      assert.strictEqual(list.size, 0);
    ...
    test('removes the specified index when multi-level traversal would be required to find it', () => {
      ...
      var list0 = PersistentList.fromArray(values);
      var list1 = PersistentList.empty().prependArray(values);
  ...
  suite('#deleteRange()', () => {
    ...
    test('returns an empty list if the index range is a superset of the list', () => {
      var list0 = PersistentList.fromArray(makeValues(BRANCH_FACTOR*3));
      var list1 = PersistentList.fromArray(makeValues(BRANCH_FACTOR >>> 1));
  ...
  suite('#[Symbol.iterator]()', () => {
    test('returns an ES6-compliant iterator', () => {
      var it = PersistentList.empty()[Symbol.iterator]();
    ...
    test('iterates through all values sequentially', () => {
      var list = PersistentList.fromArray(['X', 'Y']);
  ...
  suite('#toArray()', () => {
    test('returns an empty array if the list is empty', () => {
      assert.deepEqual(PersistentList.empty().toArray(), []);
    });
    ...
    test('returns an array of all values in a two-level list', () => {
      var values = makeValues(BRANCH_FACTOR*4);
      assert.deepEqual(PersistentList.fromArray(values).toArray(), values);
    });

    test('returns an array of all values in a three-level list', () => {
      var values = makeValues(Math.pow(BRANCH_FACTOR, 2) + BRANCH_FACTOR);
      var list = PersistentList.fromArray(values).set(BRANCH_FACTOR + 1, 'X');
