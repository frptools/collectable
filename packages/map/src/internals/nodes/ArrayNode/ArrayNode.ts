import {replaceArrayElement} from '@collectable/core';
...
export class ArrayNode<K, V> implements ListNode<K, V> {
  ...
  public modify(
    ...
    if(isEmptyNode(child) && !isEmptyNode(newChild)) {
      ...
      return new ArrayNode(group, count + 1, replaceArrayElement(fragment, newChild, children));
    }
    ...
    if(!isEmptyNode(child) && isEmptyNode(newChild)) {
      ...
      return new ArrayNode<K, V>(group, count - 1, replaceArrayElement(fragment, empty<K, V>(), children));
    }
    ...
    return new ArrayNode(group, count, replaceArrayElement(fragment, newChild, children));
  }
}
