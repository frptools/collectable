...
import {fromArray, prepend, concat} from '../../src';
import {
  createList,
  Slot,
  appendValues,
...
suite('[Internals: concatenation]', () => {
  ...
  suite('concat()', () => {
    test('joins two minimal single-node lists', () => {
      ...
      assert.deepEqual(arrayFrom(left), makeValues(3));
    });
    ...
    test('joins two single-level lists into a two-level result if capacity is exceeded', () => {
      ...
      assert.deepEqual(arrayFrom(left), makeValues(n0 + n1));
    });
    ...
    test('joins a deeper left list to a shallower right list', () => {
      ...
      assert.isTrue(left._right.parent.hasUncommittedChanges());
      assert.isTrue(left._right.parent.slot.isRelaxed());
      assert.strictEqual(left._size, n0 + n1);
      assert.deepEqual(arrayFrom(left), makeValues(n0 + n1));
    });
    ...
    test('joins lists when both lists each have pre-existing reserved head and tail views', () => {
      ...
      assert.deepEqual(arrayFrom(list1), leftValues);
      assert.deepEqual(arrayFrom(list2), rightValues);
      assert.deepEqual(arrayFrom(list3), leftValues.concat(rightValues));
    });
  });
});
