...
import { HashMapStructure, empty as emptyMap, keys as mapKeys, size as mapSize } from '@collectable/map';
...
export class SortedSetStructure<T> implements Collection<T, T[]> {
  ...
  /** @internal */
  '@@clone' (mctx: MutationContext): SortedSetStructure<T> {
    const sctx = asSubordinateContext(mctx);
    return new SortedSetStructure<T>(
      mctx,
      clone(this._map, sctx),
      clone(this._tree, sctx),
      this._compare,
      this._select
    );
  }
...
export function cloneSortedSet<T> (mutable: boolean, set: SortedSetStructure<T>, clear?: boolean): SortedSetStructure<T> {
  ...
  if(clear) {
    ...
    map = clone(set._map, sctx);
    tree = clone(set._tree, sctx);
  }
...
export function emptySet<T> (mutable: boolean|MutationContext, compare?: ComparatorFn<T>): SortedSetStructure<T>;
export function emptySet<T, K> (mutable: boolean|MutationContext, compare?: ComparatorFn<K>, select?: SelectorFn<T, K>): SortedSetStructure<T>;
export function emptySet<T, K> (mutable?: boolean|MutationContext, compare?: ComparatorFn<K>|ComparatorFn<T>, select?: SelectorFn<T, K>): SortedSetStructure<T> {
